...
class Locator:
    ...
    def __init__(self):
        ...
        def safezone_callback(safezone_data):
            ...
            x_center = safezone_data.position.x
            y_center = safezone_data.position.y
            ...
            distance = math.sqrt(math.pow((x_center - submarine_x_position),2)+math.pow((y_center - submarine_y_position),2))
        ...
        Safezone_Data = rospy.Subscriber("safezone_data", float32, safezone_callback)
        ...
        Inside_Safezone_Result = rospy.Publisher("inside_topic", float32, queue_size=10)
        Distance_From_Center = rospy.Publisher("distance_topic", float32, queue_size=10)

if __name__ == "__main__":
    #### TODO ####
    # initialize a ROS node named  "locator_node" on the line below
    node = rospy.init_node("locator_node")
